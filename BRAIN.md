...
|---|---|
| Read a page | `brain read-page <id>` |
| Read a root page | `brain read-root <slug>` |
| List pages | `brain list-pages` |
| Locate the brain | `brain brain-dir` |
| Create a page | `brain create-page --id <id> --category <category> --title "<title>"` |
| Rewrite page truth | `brain update-truth --id <id>` with body on stdin |
| Append timeline | `brain append-timeline --id <id> --kind <kind> --summary "<summary>"` |
| Archive a page | `brain archive-page --id <id>` |
| Change tags | `brain set-tags --id <id> --tags a,b` |
| Rewrite root page | `brain update-root <slug>` with body on stdin |
| Rebuild/check | `brain reindex`; `brain lint-links` |
| Wire an agent config | `brain wire --agent <claude-code|codex|opencode|cursor|pi>` |

---

## Why there is no validator — correctness by construction

There is deliberately no `validate` command. Because every write goes through the CLI, frontmatter is generated by the CLI and `update-truth` records a matching timeline entry atomically. The guarantee comes from only ever using the CLI.

**Never hand-edit a brain file.** Nothing at the file layer can stop a manual edit, and there is no validator to catch one afterwards. `brain reindex` and `brain lint-links` remain optional hygiene.

## Language

Reply in the user's working language. Write the body of timelines, compiled_truth, and root pages in the user's working language; keep technical identifiers, ids, slugs, field names, and paths verbatim.
