...
| 2026-07-26 | 待测 | 待测 | 历史回放只在结构边界重建 (ADR 0047)；本地契约 26/26 | 待测 | 待复核 `RE10110` 消除，并按观察清单复核：一类点落在趋势极值、三类点提前一个摆动至首次回试端点、离开失败并入波动区间、面板状态与点位一致 | 未执行 | 未执行 | 无 |
...
本地 `python3 -m unittest discover -s tests -v` 当前为 `26/26` 通过，在原 22 项之上新增方向感知离开判定 (ADR 0046)、MACD 前缀和、证据身份含结构时间与候选延伸不触发历史回放 (ADR 0047) 四项契约。它是源码契约检查，不替代 Pine 合成夹具或官方运行时。
