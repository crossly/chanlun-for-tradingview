...
本地源码契约能锁定关键实现约束，但仓库没有本地 Pine 编译器。进入发布验证时，仍需在 TradingView 官方编辑器用最重参考周期配置重新编译、运行 profiler，并比较实时、Bar Replay 与重载结果；spec/ADR 一致性不作为门禁。
